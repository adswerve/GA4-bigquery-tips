...
#### OBSERVATIONS #### 
#Session_start has sessions' traffic source information since the start of November 2023
...
(SELECT DISTINCT user_pseudo_id, old_ga_session_id, IF(gclid is not null, "cpc", old_medium) old_medium, old_src  FROM 
    (SELECT  user_pseudo_id,  
      ...
      FROM `analytics_1234.events_*` WHERE  _table_suffix BETWEEN FORMAT_DATE("%Y%m%d", DATE_SUB(PARSE_DATE("%Y%m%d", start_date), INTERVAL 90 DAY)) AND end_date)
),
